...
function getCodeFromBundle(entries, options) {
  let config = {
    input: entries,
    plugins: [multiEntry()]
  };
...
describe('rollup-plugin-multi-entry', () => {
  ...
  it('overrides external option as function (fix issue #20)', () =>
    getCodeFromBundle(['test/externalTest/0.js', 'test/externalTest/1.js'], {
      external: id => !/externalTest/.test(id),
      onwarn: warning => {
        throw new Error(
          'This test must not throw warning : ' + warning.message
        );
      }
    }).then(code => {
